...
# 3) Use extended regex to match either "foo" or "bar":
...
#   flags: g (global), p (print), number to replace nth match on line

## Examples
# 1) Simple replace (all occurrences on each line):
sed -e 's/foo/bar/g' file.txt

# 2) In-place replacement with backup (replace "old" with "new"):
sed -i.bak 's/old/new/g' file.txt

# 3) Delete blank lines:
sed '/^$/d' file.txt

# 4) Print only matching lines (useful with -n):
